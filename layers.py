...
class GraphIntentConv(Module):
    """
    Simple GCN layer, similar to https://arxiv.org/abs/1609.02907
    """

    def __init__(self, in_features, out_features, bias=True):
        super(GraphIntentConv, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.weight = Parameter(torch.FloatTensor(in_features, out_features))
        if bias:
        ...
        relation_weight = initializer(torch.empty(1, out_features))  # not include interact
        self.relation_weight = nn.Parameter(relation_weight)  # [n_relations - 1, in_channel]

        disen_weight_att = initializer(torch.empty(self.n_factors, 1))
        self.disen_weight_att = nn.Parameter(disen_weight_att)

        latent_emb = initializer(torch.empty(self.n_factors, in_features))
        self.latent_emb = nn.Parameter(latent_emb)
        self.reset_parameters()
    ...
    def reset_parameters(self):
        ...
        if self.bias is not None:
            self.bias.data.uniform_(-stdv, stdv)
    def forward(self, input, adj):
        score_ = torch.mm(input, self.latent_emb.t())
        score = nn.Softmax(dim = 1)(score_).unsqueeze(-1)#(n_nodes, n_factors, 1)

        support = torch.mm(input, self.weight)#(n_nodes, out_channel)
        output = torch.spmm(adj, support)#(n_nodes, out_channel)

        # import pdb;pdb.set_trace()
        disen_weight = torch.mm(nn.Softmax(dim=-1)(self.disen_weight_att),
                               self.relation_weight).expand(len(adj), self.n_factors, self.out_features)#(n_nodes, n_factors, out_channel)
        output = output * (disen_weight * score).sum(dim=1) + output #(n_nodes, out_channel)
        if self.bias is not None:
            return output + self.bias
        else:
            return output
...
class GNN_Encoder(nn.Module):
    def __init__(self, layer, nfeat, nhid, dropout, nhead=1, adj=None):
        ...
        if layer == 'gcn':
            ...
            self.conv2 = GraphAttConv(nhid, nhid, nhead, dropout)
...
class GraphIntentAttConv(nn.Module):
    def __init__(self, in_features, out_features, heads, dropout):
        super(GraphIntentAttConv, self).__init__()
        assert out_features % heads == 0
        out_perhead = out_features // heads

        self.graph_atts = nn.ModuleList([GraphIntentAttConvOneHead(
               in_features, out_perhead, dropout=dropout) for _ in range(heads)])
...
class GraphIntentAttConvOneHead(nn.Module):
    """
    Sparse version GAT layer, single head
    """

    def __init__(self, in_features, out_features, dropout=0.6, alpha=0.2):
        super(GraphIntentAttConvOneHead, self).__init__()
        self.weight = Parameter(torch.zeros(size=(in_features, out_features)))
        self.a = Parameter(torch.zeros(size=(1, 2*out_features)))
        # init 
        nn.init.xavier_normal_(self.weight.data, gain=nn.init.calculate_gain('relu')) # look at here
...
class GIN_Encoder(nn.Module):
    def __init__(self, layer, nfeat, nhid, dropout, nhead=1, adj=None):
        ...
        if layer == 'gcn':
            self.conv = GraphIntentConv(nfeat, nhid)
            self.activation = nn.ReLU()
        elif layer == 'gat':
            self.conv = GraphIntentAttConv(nfeat, nhid, nhead, dropout)
            self.activation = nn.ELU()
...
class GIN_Classifier(nn.Module):
    def __init__(self, layer, nhid, nclass, dropout, nhead=1, adj=None):
        ...
        if layer == 'gcn':
            ...
            self.conv = GraphIntentAttConv(nhid, nhid, nhead, dropout)
            self.activation = nn.ELU(True)
