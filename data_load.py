...
def generate_tail_edges(head_edges):
    head_edges = head_edges.t().numpy()
    degree = dict()
    for srt, dst in head_edges:
        if srt not in degree:
            degree[srt] = 1
        else:
            degree[srt] += 1
    ...
    for srt, dst in head_edges:
        if temp_srt == srt:
            temp_edges.append([srt, dst])
        else:
            temp_srt = srt
            if len(temp_edges) > 5:
                random.shuffle(temp_edges)
                tail_edges.extend(temp_edges[:5])
            else:
                tail_edges.extend(temp_edges)
            temp_edges = []
    tail_edges = torch.tensor(tail_edges).t()
    return tail_edges
...
class Dataset(InMemoryDataset):
    ...
    def __init__(self, root="data", dataset='cora', transform=None, pre_transform=None, is_normalize=True, add_self_loop=False):
        ...
        tail_edges = generate_tail_edges(self.data.edge_index)
        # import pdb;pdb.set_trace()
        ...
        tail_adj = sp.coo_matrix((np.ones(tail_edges.shape[1]), (tail_edges[0,:], tail_edges[1,:])), shape=(self.data.y.shape[0], self.data.y.shape[0]), dtype=np.float32)
        self.adj = utils.sparse_mx_to_torch_sparse_tensor(adj)
        self.tail_adj = utils.sparse_mx_to_torch_sparse_tensor(tail_adj)
