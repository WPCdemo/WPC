...
def split_manual(labels, c_train_num, idx_map, min_class_num):
    ...
    train_min_idx = []
    max_num = 0
    min_num = c_train_num[num_classes-1]
    ...
    for i in range(num_classes):
        ...
        random.shuffle(c_idx)
        c_idxs.append(c_idx)
        c_num_mat[i,1] = int(min_class_num*0.1)
        if c_train_num[i] + c_num_mat[i,1] + c_num_mat[i,1] >len(c_idx):
            c_num_mat[i,2] = len(c_idx) - c_num_mat[i,1] - c_train_num[i]
        else:
            c_num_mat[i,2] = c_num_mat[i,1]
        train_idx = train_idx + c_idx[:c_train_num[i]]
        ##################### train_max_idx
        if i==0:
            train_max_idx = train_max_idx + c_idx[:c_train_num[i]]
            max_num = c_train_num[i]
        else:
            temp_max_num = max_num
            while temp_max_num > 0:
                if temp_max_num > c_train_num[i]:
                    train_max_idx = train_max_idx + c_idx[:c_train_num[i]]
                    temp_max_num -= c_train_num[i]
                else:
                    train_max_idx = train_max_idx + c_idx[:temp_max_num]
                    temp_max_num -= c_train_num[i]
        ##################### train_min_idx
        train_min_idx = train_min_idx + c_idx[:min_num]


        
        if c_train_num[i] < 500:
            part_train_idx = part_train_idx + c_idx[:c_train_num[i]]
        else:
            part_train_idx = part_train_idx + c_idx[:int(c_train_num[i]/10)]
    ...
    random.shuffle(part_train_idx)
    random.shuffle(train_min_idx)
    random.shuffle(train_max_idx)

    train_idx = torch.LongTensor(train_idx)
    part_train_idx = torch.LongTensor(part_train_idx)
    train_min_idx = torch.LongTensor(train_min_idx)
    train_max_idx = torch.LongTensor(train_max_idx)
    val_idx = torch.LongTensor(val_idx)
    test_idx = torch.LongTensor(test_idx)

    return train_idx, part_train_idx, train_min_idx, train_max_idx, val_idx, test_idx, c_num_mat
...
def split_natural(labels, idx_map):
    ...
    for i in range(num_classes):
        ...
        if c_num == 3:
            ...
            c_num_mat[i,1] = int(c_num*0.4) # 10% for validation
            c_num_mat[i,2] = int(c_num*0.5) # 80% for test
